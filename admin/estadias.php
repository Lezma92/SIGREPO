    ...
    <div id="wrapper">
      ...
      <div class="container-fluid " style="margin-bottom: 85px;">
        ...
        <ol class="breadcrumb" style="margin-top: 10px">
          ...
          <li class="breadcrumb-item active">Proyectos de estadías</li>
        </ol>
        ...
        <div class="card-body misael" style="padding: 2px;">

          <?php include("../modelo/consultas.php"); ?>

          <?php
          $res = vistas::resultadoTotal($categoria, "Estadía");
          $articulosxpagina = 6;
          $totalResult = count($res);
          $totalPaginas = ceil($totalResult / 6);
          $mostrarPag = ($_GET["pag"] - 1) * $articulosxpagina;
          $verResultados = vistas::getTdosProyectos($categoria, "Estadía", $mostrarPag, $articulosxpagina);
                    ...
                    <p class="card-text"><small class="text-muted">
                        ...
                        <form method="POST">
                          ...
                          <?php if ($_SESSION["nivel"] == "Administrador") { ?>
                            <button type="button" name="eliminar" onclick="eliminarRepositorios(<?php echo ($value["idProyecto"]); ?>,<?php echo ($value["matricula"]); ?>,'<?php echo ($value["nombre_proyecto"]); ?>',2,'<?php echo ($_GET["cat"]); ?>',<?php echo ($_GET["pag"]); ?>);" id="eliminar" class="btn  bg-danger">
                              Eliminar
                            </button>
                          <?php } ?>
                        </form>
        ...
        <nav aria-label="Page navigation example">
          <ul class="pagination">
            <li class="page-item <?php echo ($_GET["pag"] <= $totalPaginas ? "disabled" : "")
                                  ?> ">
              <a class="page-link" href="integradora.php?cat=<?php echo ($_GET["cat"]); ?>&pag=<?php echo ($_GET["pag"] - 1); ?>">Anterior</a>
            </li>
            ...
            for ($i = 0; $i < $totalPaginas; $i++) { ?>
              ...
              <li class="page-item <?php echo ($_GET["pag"] == $i + 1 ? "active" : ""); ?>">
                <a class="page-link " href="integradora.php?cat=<?php echo ($_GET["cat"]); ?>&pag=<?php echo ($i + 1); ?>"><?php echo ($i + 1) ?></a>
              </li>
            <?php            }              ?>

            <li class="page-item <?php echo ($_GET["pag"] >= $totalPaginas ? "disabled" : "") ?> "><a class="page-link" href="integradora.php?cat=<?php echo ($_GET["cat"]); ?>&pag=<?php echo ($_GET["pag"] + 1); ?>">Siguiente</a>
            </li>
          </ul>
        </nav>
